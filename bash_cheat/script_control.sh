...
# If the script receives a signal listed in the trap command, it prevents it from being processed by the shell and instead handles it locally:
    ...
    $Green$ trap cmd sig1 sig2$colorOff     # executes a command when a signal is received by the script
    $Green$ trap \"\" sig1 sig2$colorOff      # ignores that signals
    $Green$ trap - sig1 sig2$colorOff       # resets the action taken when the signal is received to the default
...
examples(){
    clear
    echo -e "
    $Bold[+] Use full Functions with trap command$colorOff

${Green}function errtrap {
  es=\$?
  echo \"ERROR line \$1: Command exited with status \$es.\"
}
trap 'errtrap \$LINENO' ERR$colorOff     # is run whenever a command in the surrounding script or function exits with non-zero status

${Green}function dbgtrap {
  echo \"badvar is \$badvar\"
}
trap dbgtrap DEBUG$colorOff  # causes the trap code to be executed before every statement in a function or script


# trap - DEBUG  # turn off the DEBUG trap

${Green}function returntrap {
  echo 'A return occurred'
}
trap returntrap RETURN $colorOff # is executed each time a shell function or a script executed with the . or source commands finishes executing
"
}
#
#
...
echo -e "$Green$ at -f tstAtCmd.sh now +2$colorOff"
at -f "$path/tstAtCmd.sh" now +2 minutes
...
echo -e "\t$Green$ at -M -f tstAtCmd.sh now$colorOff  # -M : to suppress any output generated by jobs using the at command."
...
echo -e "$Bold[+] Scheduling a job using the at command:$colorOff
...
$Bold[+] Welcome to Script Controle:$colorOff
    ...
    1. Handling Signal
    2. Use Full trap functions.
    3. Nice Command
    4. Running Like Clockwork
    0. Exit"
